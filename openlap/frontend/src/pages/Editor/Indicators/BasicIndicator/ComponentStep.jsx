...
import { styled, Alert, Grid } from "@mui/material";
import {
  Box,
  Step,
  StepLabel,
...
} from "../../../../utils/ruleEngine/ruleGenerator";
 ...
 */
export default function ComponentStep(props, { classes }) {
...
// and on deselect call the delete function
  ...
  const completedStep = useSelector(
    (state) => state.indicatorEditorReducer.common.completedStep
  );
  const selectedAnalysisMethod = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.analysisMethod
  );
  const selectedVisualizationMethodsAndTypes = useSelector(
    (state) =>
      state.indicatorEditorReducer.selectedData.visualizationMethodsAndTypes
  );
  const selectedVisualizationMapping = useSelector(
    (state) =>
      state.indicatorEditorReducer.selectedData.mappingVizInputAnalysisOutput
  );
  const selectedPlatform = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.platforms
  );
  const selectedActivityName = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.activityName
  );
  const selectedActivityTypes = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.activityTypes
  );
  const selectedActionOnActivities = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.actionOnActivities
  );
  const selectedActivityExtensionData = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.activityExtensionId
  );
  const selectedContextActivities = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.contextActivities
  );
  const selectedResult = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.result
  );
  const timeDurationData = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.timeDuration
  );
  const selectedMappingAnalysisInputAttributesData = useSelector(
    (state) =>
      state.indicatorEditorReducer.selectedData
        .mappingAnalysisInputAttributesData
  );
  const selectedAnalyticsMethodParams = useSelector(
    (state) => state.indicatorEditorReducer.selectedData.analysisMethodParams
  );
  ...
  const handleOpenVisualization = () => {
    ...
    let methodInputParams = JSON.stringify(
      selectedAnalyticsMethodParams.reduce((obj, item) => {
    ...
    let getIndicatorPreviewCode = {
      ...
      queryToMethodConfig: {
        0: { mapping: selectedMappingAnalysisInputAttributesData },
      },
  ...
  useEffect(() => {
    !completed.includes(STEPS[activeStep]?.name)
  ...
  const STEPS = [
    ...
    // todo: add the logic of viewing the preview at this steppp
    {
      name: "Preview",
      component: <_preview />,
      color: "#0094a4",
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: "16px" }}>
          All steps are completed. Please name your indicator and save it on the
          bottom to your workspace.
        </span>
      </div>
    </div>
  );
  ...
  const _customStepIcon = (props) => {
    ...
    const ColoredStepIcon = styled("div")(
      ({ theme, active, completed, error, color }) => ({
        width: 24,
        height: 24,
        display: "flex",
        alignItems: "center",
