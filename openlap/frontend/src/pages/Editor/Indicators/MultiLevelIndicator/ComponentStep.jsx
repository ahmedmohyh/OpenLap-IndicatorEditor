...
import {
  generateMultiLevelIndicatorPreview,
  saveMultiLevelIndicatorPreview,
  setGeneratedMultiLevelVisualizationCode,
...
export default function ComponentStep(props) {
  ...
  const selectIndicators = useSelector(
    (state) => state.multiLevelEditorReducer.selectedData.indicators
  );
  ...
  const indRefParams = useSelector(
    (state) => state.multiLevelEditorReducer.selectedData.indRefParams
  );
  const analysisMethodOutputsData = useSelector(
    (state) => state.multiLevelEditorReducer.fetchedData.analysisMethodOutputs
  );
  ...
  const handleOpenVisualization = () => {
    handleStepForward();
    dispatch(setGeneratedMultiLevelVisualizationCode("", ""));
    ...
    let _queryToMethodConfig = {
      0: {
        mapping: selectedMappingAnalysisInputAttributesData,
      },
    };
    let _analyticsMethodId = { 0: selectedAnalysisMethod[0].id };
    let _visualizationInputParams = {};
    let _params = { 0: methodInputParams };
    for (let index in Object.keys(basicIndicatorPreviewMap)) {
      if (
        selectIndicators.some(
          (v) => v.id === Object.keys(basicIndicatorPreviewMap)[index]
        )
      ) {
        _params[`${Object.keys(_params).length}`] = JSON.stringify(
          Object.values(basicIndicatorPreviewMap)[index]["methodParams"]
        );
    ...
    let getMultiLevelIndicatorPreviewCode = {
      queries: _queries,
      indicatorType: "multianalysis",
      ...
      dataSetMergeMappingList: indRefParams,
      createdBy: createdBy,
    };
    dispatch(
      generateMultiLevelIndicatorPreview(getMultiLevelIndicatorPreviewCode)
    );
    dispatch(saveMultiLevelIndicatorPreview(getMultiLevelIndicatorPreviewCode));
        ...
        {STEPS.length !== activeStep ? (
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: "16px" }}>
                All steps are completed. Please name your indicator and save it
                on the bottom to your workspace.
              </span>
            </div>
          </div>
        )}
