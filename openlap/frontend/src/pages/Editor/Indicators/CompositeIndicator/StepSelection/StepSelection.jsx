...
export default function StepSelection(props) {
  const { setAnalysisMethodOutputsData, onStepComplete, onStepIncomplete } =
    props;
  ...
  const checkIndicatorCompatibility = (selections) => {
    ...
    for (let s in selections) {
      let tempIndicatorObj = basicIndicatorPreviewList.find(
        (obj) => obj.id === selections[s].id
      );
      ...
      for (let i in basicIndicatorPreviewList) {
        ...
        if (
          ...
          basicIndicatorPreviewList[i]["analyticsMethodId"] !==
            tempIndicatorObj["analyticsMethodId"]
        ) {
  ...
  const handleChange = (event) => {
    ...
    let indicatorObj = basicIndicatorPreviewList.find(
      (v) => v.id === indicatorId
    );
      ...
      >
        {basicIndicatorPreviewList.length > 0 ? (
          <div>
            ...
            <Grid item md={12} style={{ maxHeight: "700px" }}>
              <SelectContainer
                name={"Indicators"}
                isMandatory={true}
